...
(function($){
	...
	$.extend(WPSC_REPORTS_Page, /** @lends WPSC_REPORTS_Page */ {
		...
		init : function() {
			...
			$(function(){
				...
				wpsc_options.on( 'submit', '#wpsc-settings-form'    , WPSC_REPORTS_Page.event_settings_form_submitted);
		...
		load_tab : function(url, push_state) {
			...
			var post_data = $.extend({}, query.get(), {
				...
				'nonce'       : WPSC_REPORTS_Page.navigate_settings_tab_nonce,
	...
	WPSC_REPORTS_Page.General = {
		...
		event_init : function() {
			...
			wrapper.on( 'change', '#wpsc-base-country-drop-down', WPSC_REPORTS_Page.General.event_base_country_changed);
			wrapper.on( 'click' , '.wpsc-select-all'            , WPSC_REPORTS_Page.General.event_select_all);
			wrapper.on( 'click' ,'.wpsc-select-none'            , WPSC_REPORTS_Page.General.event_select_none);
		},
		...
		event_base_country_changed : function() {
			...
			var postdata = {
				...
				nonce   : WPSC_REPORTS_Page.display_region_list_nonce
			};
	...
	$(WPSC_REPORTS_Page).on('WPSC_REPORTS_tab_loaded_general', WPSC_REPORTS_Page.General.event_init);
	 ...
	 * @since 3.8.8
	 */
	WPSC_REPORTS_Page.Presentation = {
		/**
		...
		event_init : function() {
			var wrapper = $('#options_presentation'),
				checkbox_selector = '#' + WPSC_REPORTS_Page.Presentation.grid_view_boxes.join(',#');
			wrapper.on( 'click', '#wpsc-show-images-only', WPSC_REPORTS_Page.Presentation.event_show_images_only_clicked);
			wrapper.on( 'click', checkbox_selector       , WPSC_REPORTS_Page.Presentation.event_grid_view_boxes_clicked);
		},
		...
		event_show_images_only_clicked : function() {
			var i;
			if ($(this).is(':checked')) {
				for (i in WPSC_REPORTS_Page.Presentation.grid_view_boxes) {
					document.getElementById(WPSC_REPORTS_Page.Presentation.grid_view_boxes[i]).checked = false;
				}
			}
		}
	};
	$(WPSC_REPORTS_Page).on('WPSC_REPORTS_tab_loaded_presentation', WPSC_REPORTS_Page.Presentation.event_init);
	...
	WPSC_REPORTS_Page.Checkout = {
		...
		event_init : function() {
			...
			wrapper.on( 'click', '.add_new_form_set', WPSC_REPORTS_Page.Checkout.event_add_new_form_set);
			wrapper.on( 'click', '.actionscol a.wpsc-button-plus', WPSC_REPORTS_Page.Checkout.event_add_new_field);
			wrapper.on( 'click', '.actionscol a.wpsc-button-minus', WPSC_REPORTS_Page.Checkout.event_delete_field);
			wrapper.on( 'click', 'a.edit-options', WPSC_REPORTS_Page.Checkout.event_edit_field_options);
			wrapper.on( 'change', 'select[name^="form_type"], select[name^="new_field_type"]', WPSC_REPORTS_Page.Checkout.event_form_type_changed);
			wrapper.on( 'click', '.field-option-cell-wrapper .wpsc-button-plus', WPSC_REPORTS_Page.Checkout.event_add_field_option);
			wrapper.on( 'click', '.field-option-cell-wrapper .wpsc-button-minus', WPSC_REPORTS_Page.Checkout.event_delete_field_option);
			wrapper.on( 'click', '#wpsc-delete-checkout-set', WPSC_REPORTS_Page.Checkout.event_delete_checkout_set);
			wrapper.on( 'change', '#wpsc_form_set', WPSC_REPORTS_Page.Checkout.event_select_form_set);
			$('#wpsc-settings-form').on( 'submit', WPSC_REPORTS_Page.Checkout.event_form_submit);
			...
			wrapper.find('#wpsc_checkout_list').
				sortable({
					...
					handle      : '.drag',
					sort        : WPSC_REPORTS_Page.Checkout.event_sort,
					helper      : WPSC_REPORTS_Page.Checkout.fix_sortable_helper,
					start       : WPSC_REPORTS_Page.Checkout.event_sort_start,
					stop        : WPSC_REPORTS_Page.Checkout.event_sort_stop,
					update      : WPSC_REPORTS_Page.Checkout.event_sort_update
				});

			WPSC_REPORTS_Page.Checkout.new_field_count = $('.new-field').length;
		},
		...
		event_form_type_changed : function() {
			...
			if ($.inArray(t.val(), ['select', 'radio', 'checkbox']) !== -1) {
				...
				link.hide().text(WPSC_REPORTS_Page.edit_field_options).removeClass('expanded');
		...
		event_edit_field_options : function() {
			...
			if (t.hasClass('expanded')) {
				...
				t.text(WPSC_REPORTS_Page.edit_field_options);
				return false;
			}

			t.addClass('expanded');
			t.text(WPSC_REPORTS_Page.hide_edit_field_options);
		...
		event_add_new_field : function() {
			...
			WPSC_REPORTS_Page.Checkout.new_field_count ++;
			id = WPSC_REPORTS_Page.Checkout.new_field_count;
			new_row.
		...
		event_delete_field : function() {
			...
			if ( $('.checkout_form_field').length == 1 ) {
				...
				target_row.find('.edit-options').removeClass('expanded').text(WPSC_REPORTS_Page.edit_field_options).hide();
		...
		event_sort_update : function(e, ui) {
			...
			var post_data = {
				...
				nonce      : WPSC_REPORTS_Page.update_checkout_fields_order_nonce,
		...
		event_delete_checkout_set : function() {
			var dropdown = $('#wpsc_form_set'),
				...
				message = WPSC_REPORTS_Page.delete_form_set_confirm.replace('%s', form_name);
	...
	$(WPSC_REPORTS_Page).on('WPSC_REPORTS_tab_loaded_checkout', WPSC_REPORTS_Page.Checkout.event_init);
	...
	WPSC_REPORTS_Page.Taxes = {
		...
		event_init : function() {
			...
			wrapper.on( 'click' , '.wpsc-taxes-rates-add'        , WPSC_REPORTS_Page.Taxes.event_add_tax_rate);
			wrapper.on( 'click' , '.wpsc-taxes-rates-delete'     , WPSC_REPORTS_Page.Taxes.event_delete_tax_rate);
			wrapper.on( 'click' , '.wpsc-taxes-bands-add'        , WPSC_REPORTS_Page.Taxes.event_add_tax_band);
			wrapper.on( 'click' , '.wpsc-taxes-bands-delete'     , WPSC_REPORTS_Page.Taxes.event_delete_tax_band);
			wrapper.on( 'change', '.wpsc-taxes-country-drop-down', WPSC_REPORTS_Page.Taxes.event_country_drop_down_changed);
		},
		...
		event_country_drop_down_changed : function() {
			var c = $(this),
				post_data = {
					...
					country_code      : c.val(),
					nonce             : WPSC_REPORTS_Page.add_tax_rate_nonce
				},
		...
		event_add_tax_rate : function() {
			WPSC_REPORTS_Page.Taxes.add_field('rates');
			return false;
		},
		...
		event_add_tax_band : function() {
			WPSC_REPORTS_Page.Taxes.add_field('bands');
			return false;
		},
	...
	$(WPSC_REPORTS_Page).on('WPSC_REPORTS_tab_loaded_taxes', WPSC_REPORTS_Page.Taxes.event_init);
	 ...
	 * @since 3.8.8
	 */
	WPSC_REPORTS_Page.Shipping = {
		/**
		 * Event binding for Shipping tab.
		 * @since 3.8.8
		 */
		event_init : function() {
			WPSC_REPORTS_Page.Shipping.wrapper = $('#options_shipping');
			WPSC_REPORTS_Page.Shipping.table_rate = WPSC_REPORTS_Page.Shipping.wrapper.find('.table-rate');

			WPSC_REPORTS_Page.Shipping.wrapper.on( 'click'   , '.edit-shipping-module'         , WPSC_REPORTS_Page.Shipping.event_edit_shipping_module);
			WPSC_REPORTS_Page.Shipping.wrapper.on( 'click'   , '.table-rate .wpsc-button-plus' , WPSC_REPORTS_Page.Shipping.event_add_table_rate_layer);
			WPSC_REPORTS_Page.Shipping.wrapper.on( 'click'   , '.table-rate .wpsc-button-minus', WPSC_REPORTS_Page.Shipping.event_delete_table_rate_layer);
			WPSC_REPORTS_Page.Shipping.wrapper.on( 'keypress', '.table-rate input[type="text"]', WPSC_REPORTS_Page.Shipping.event_enter_key_pressed);
			WPSC_REPORTS_Page.Shipping.wrapper.on( 'click'   , 'a.shipwire_sync'               , WPSC_REPORTS_Page.Shipping.event_sync_shipwire);
			WPSC_REPORTS_Page.Shipping.wrapper.on( 'click'   , '.edit-shipping-module-cancel'  , WPSC_REPORTS_Page.Shipping.event_edit_shipping_module_cancel);
		},
		...
		event_add_table_rate_layer : function(e, focus_on_new_row) {
			...
			WPSC_REPORTS_Page.Shipping.refresh_alt_row();
			return false;
		},
		...
		event_delete_table_rate_layer : function() {
			var this_row = $(this).closest('tr');
			if (WPSC_REPORTS_Page.Shipping.wrapper.find('.table-rate tr:not(.js-warning)').size() == 1) {
		...
		event_edit_shipping_module : function() {
			var element = $(this),
				...
				post_data = {
					...
					nonce  : WPSC_REPORTS_Page.shipping_module_settings_form_nonce
				},
				ajax_callback = function(response) {
					if (! response.is_successful) {
						alert(response.error.messages.join("\n"));
					...
					if (history.pushState) {
						var new_url = '?page=wpsc-settings&tab=' + WPSC_REPORTS_Page.current_tab + '&shipping_module_id=' + shipping_module_id;
		...
		event_edit_shipping_module_cancel : function() {
			...
			if (history.pushState) {
				var new_url = '?page=wpsc-settings&tab=' + WPSC_REPORTS_Page.current_tab;
				history.pushState({'url' : new_url}, '', new_url);
			}
			$('#wpsc_shipping_settings_' + shipping_module_id + '_form').remove();
		...
		refresh_alt_row : function() {
			WPSC_REPORTS_Page.Shipping.wrapper.find('.alternate').removeClass('alternate');
			WPSC_REPORTS_Page.Shipping.wrapper.find('#wpsc-shipping-module-settings tr:odd').addClass('alternate');
		}
	};
	$(WPSC_REPORTS_Page).on('WPSC_REPORTS_tab_loaded_shipping', WPSC_REPORTS_Page.Shipping.event_init);
	 ...
	 * @since 3.8.8
	 */
	WPSC_REPORTS_Page.Gateway = {
		event_init : function() {
			var wrapper = $('#options_gateway');
			wrapper.on( 'click', '.edit-payment-module'       , WPSC_REPORTS_Page.Gateway.event_edit_payment_gateway);
			wrapper.on( 'click', '.edit-payment-module-cancel', WPSC_REPORTS_Page.Gateway.event_edit_payment_gateway_cancel);
		},
		event_edit_payment_gateway : function() {
			var element = $(this), // edit link
				...
				post_data = {
					...
					nonce                : WPSC_REPORTS_Page.payment_gateway_settings_form_nonce
				},
				ajax_callback = function(response) {
					if (! response.is_successful) {
						alert(response.error.messages.join("\n"));
					...
					if (history.pushState) {
						var new_url = '?page=wpsc-settings&tab=' + WPSC_REPORTS_Page.current_tab + '&payment_gateway_id=' + payment_gateway_id;
						history.pushState({'url' : new_url}, '', new_url);
					}
					spinner.toggleClass('ajax-feedback-active');
	...
	$(WPSC_REPORTS_Page).on('WPSC_REPORTS_tab_loaded_gateway', WPSC_REPORTS_Page.Gateway.event_init);
	 ...
	 * @since 3.8.9
	 */
	WPSC_REPORTS_Page.Marketing = {
		event_init : function() {

			var wrapper = $('div#wpsc_google_analytics_integration');

			wrapper.on( 'click', 'input[type="checkbox"]', WPSC_REPORTS_Page.Marketing.event_show_hide_dependencies);
	...
	$(WPSC_REPORTS_Page).on('WPSC_REPORTS_tab_loaded_marketing', WPSC_REPORTS_Page.Marketing.event_init);
